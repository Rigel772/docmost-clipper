...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  buttons.clip.addEventListener('click', async () => {
    const selectedValue = inputs.spaceSelect.value;
    if (!selectedValue || selectedValue === '__NEW_SPACE__') {
    ...
    try {
      ...
      // Save the last used space ID (or calendar option)
      await chrome.storage.local.set({ lastSpaceId: selectedValue });
  ...
  async function movePage(baseUrl, pageId, parentPageId, spaceId) {
    ...
    // parentPageId here is the container we are moving INTO, so we want its children.
    const siblings = await fetchPagesInSpace(baseUrl, spaceId, parentPageId);

    // No need to filter by parentPageId because fetchPagesInSpace(..., parentPageId) returns exactly that.
