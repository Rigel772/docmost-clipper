...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const stored = await browser.storage.local.get(['docmostUrl', 'lastSpaceId', 'theme']);
  ...
  function handleApiError(error, retryCallback) {
    ...
    if (error.message.includes('401') || error.message.includes('403')) {
      showStatus(browser.i18n.getMessage('statusSessionExpired'), 'error');
      toggleLoginState(false);
      showView('settings');
      return;
    }
    ...
    showStatus(error.message || browser.i18n.getMessage('statusNetworkError'), 'error');
  ...
  buttons.retry.addEventListener('click', () => {
    if (retryAction) {
      showStatus(browser.i18n.getMessage('statusRetrying'), 'success');
  ...
  buttons.saveSettings.addEventListener('click', async () => {
    ...
    if (!rawUrl) {
      showStatus(browser.i18n.getMessage('errorEnterUrl'), 'error');
      return;
    }
    ...
    try {
      ...
      if (urlObj.protocol !== 'https:' && urlObj.hostname !== 'localhost' && urlObj.hostname !== '127.0.0.1') {
        showStatus(browser.i18n.getMessage('errorHttpsRequired'), 'error');
        return;
      }

      // Path Check (Must be root)
      if (urlObj.pathname !== '/' && urlObj.pathname !== '') {
        showStatus(browser.i18n.getMessage('errorInvalidUrlPath'), 'error');
        return;
      }
      ...
      showStatus(browser.i18n.getMessage('errorInvalidUrlFormat'), 'error');
      return;
    }

    // Security: Host Change Warning
    // We get the stored URL again to be sure
    const freshStored = await browser.storage.local.get(['docmostUrl']);
      ...
      try {
        ...
        if (oldUrlObj.hostname !== newHostname) {
          if (pendingUrlChange !== newHostname) {
            ...
            showStatus(browser.i18n.getMessage('warnHostChange'), 'error');
    ...
    if (!email || !password) {
      showStatus(browser.i18n.getMessage('errorEnterEmail'), 'error');
      return;
    }

    buttons.saveSettings.disabled = true;
    buttons.saveSettings.textContent = browser.i18n.getMessage('statusConnecting');
    showStatus(browser.i18n.getMessage('statusConnecting'), 'success');
    hideRetry();
    ...
    try {
      ...
      showStatus(browser.i18n.getMessage('statusConnected'), 'success');
      initializeClipView();

    } catch (err) {
      console.error(err);
      if (err.message.includes('Login Error') || err.message.includes('401')) {
        showStatus(browser.i18n.getMessage('errorLoginFailed'), 'error');
      } else {
        handleApiError(err, () => buttons.saveSettings.click());
      }
    } finally {
      buttons.saveSettings.disabled = false;
      buttons.saveSettings.textContent = browser.i18n.getMessage('btnConnect');
    }
  });
  ...
  buttons.disconnect.addEventListener('click', async () => {
    ...
    showStatus(browser.i18n.getMessage('statusDisconnected'), 'success');
    hideRetry();
  });
  ...
  buttons.cancelCreateSpace.addEventListener('click', () => {
    ...
    browser.storage.local.get(['lastSpaceId']).then((stored) => {
  ...
  buttons.confirmCreateSpace.addEventListener('click', async () => {
    ...
    if (!name || name.length < 2) {
      showStatus(browser.i18n.getMessage('errorNameLength'), 'error');
      return;
    }

    buttons.confirmCreateSpace.disabled = true;
    buttons.confirmCreateSpace.textContent = browser.i18n.getMessage('statusCreating');
    hideRetry();

    try {
      const { docmostUrl } = await browser.storage.local.get(['docmostUrl']);
      ...
      showStatus(browser.i18n.getMessage('statusSpaceCreated'), 'success');
      ...
      buttons.confirmCreateSpace.textContent = browser.i18n.getMessage('btnCreateSpace');
    }
  });
  ...
  buttons.clip.addEventListener('click', async () => {
    ...
    if (!spaceId || spaceId === '__NEW_SPACE__') {
      showStatus(browser.i18n.getMessage('errorSelectSpace'), 'error');
      return;
    }

    if (!currentContentData) {
      showStatus(browser.i18n.getMessage('errorNoContent'), 'error');
      return;
    }

    buttons.clip.disabled = true;
    buttons.clip.textContent = browser.i18n.getMessage('statusClipping');
    ...
    try {
      ...
      showStatus(browser.i18n.getMessage('statusClipped'), 'success');
      ...
      buttons.clip.textContent = browser.i18n.getMessage('btnClip');
    }
  });
  ...
  async function initializeClipView() {
    try {
      ...
      console.error("Failed to initialize clip view", e);
    }
  }

  function populateSpaces(spaces, selectedId = null) {
    // Clear existing options
    while (inputs.spaceSelect.firstChild) {
      inputs.spaceSelect.removeChild(inputs.spaceSelect.firstChild);
    }
    ...
    placeholderOpt.textContent = browser.i18n.getMessage('optSelectSpace');
    inputs.spaceSelect.appendChild(placeholderOpt);
    ...
    spaces.forEach(space => {
      ...
      opt.value = space.id;
      opt.textContent = space.name || space.title || space.slug || browser.i18n.getMessage('optUnnamedSpace');
  ...
  async function sendMessageToTab(tabId, message) {
    try {
      ...
      console.warn('Docmost Clipper: Message failed, attempting injection...', error);

      // In Firefox, if the tab is just loading or valid but no script, we assume we need to inject.
      // We catch ALL errors here to be safe, because "Receiving end does not exist" is just one variant.
      try {
        ...
        console.error('Docmost Clipper: Injection failed', injectionError);
  ...
  async function getAntiCsrfHeaders(baseUrl) {
    const headers = {};
    if (!browser.cookies) return headers;
    ...
    try {
      ...
      for (const c of candidates) {
        const cookie = await browser.cookies.get({ url: baseUrl, name: c.cookie });
        if (cookie) {
